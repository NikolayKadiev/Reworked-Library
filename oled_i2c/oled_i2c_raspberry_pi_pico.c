...
{
    uint8_t msg_send[2];
    msg_send[0]= SSD1306_DATA;
    msg_send[1]= ch;
    i2c_write_blocking(i2c_default, SSD1306_ADDRESS, msg_send, 2, false);

	// i2c_cmd_handle_t cmd_send;
	// cmd_send = i2c_cmd_link_create();
	// i2c_master_start(cmd_send);
	// i2c_master_write_byte(cmd_send, ( SSD1306_ADDRESS << 1 ) | I2C_MASTER_WRITE, 1);
	// i2c_master_write_byte(cmd_send, SSD1306_DATA , 1);
	// i2c_master_write_byte(cmd_send, ch, 1);//oledSendByte(SSD1306_COMMAND);
	// i2c_master_stop(cmd_send);
	// i2c_master_cmd_begin(I2C_NUM_1, cmd_send, portMAX_DELAY);
	// i2c_cmd_link_delete(cmd_send);
}


void oledSendCommand(uint8_t oled_cmd)
{
    uint8_t msg_send[2];
    msg_send[0]= SSD1306_COMMAND;
    msg_send[1]= oled_cmd;
    i2c_write_blocking(i2c_default, SSD1306_ADDRESS, msg_send, 2, false);
	...
	// i2c_master_write_byte(cmd_comm, ( SSD1306_ADDRESS << 1 ) | I2C_MASTER_WRITE, 1);//oledSendStart(SSD1306_ADDRESS<<1);
	// i2c_master_write_byte(cmd_comm, SSD1306_COMMAND , 1);//oledSendByte(SSD1306_COMMAND);
	// i2c_master_write_byte(cmd_comm, oled_cmd , 1);//oledSendByte(oled_cmd);
	// i2c_master_stop(cmd_comm);//oledSendStop();
